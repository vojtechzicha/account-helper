...
export const calculateSocialActions = async (inv, conf) => {
  ...
  const totalPrepaymentToBePaid = currentPrepayments.map(p => Math.ceil(p.amount / diff(inv.date, p.date))).reduce((p, c) => p + c, 0),
    ...
    minimalDesiredDeposit = calcs[0].totalSocial - calcs[0].totalPaid - calcs[0].upcomingPrepayments.reduce((p, c) => p + c.amount, 0),
...
const getSocialCalculation = async (inv, year, conf, balances = true) => {
  ...
  const totalPaid = (await getTransfers(year, 'SI', conf.ynab)).reduce((p, c) => p + c.amount, 0),
    upcomingPrepayments = [...conf.social[year].prepayments.filter(p => isBefore(inv.date, p.date))]
